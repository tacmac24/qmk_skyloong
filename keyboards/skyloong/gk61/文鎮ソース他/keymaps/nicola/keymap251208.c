...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
     ...
     * ├───┼──┼──┼──┼──┼──┼──┼──┼──┼──┼──┼──┼─────┤
     * │Sft   │ Z  │ X  │ C  │ V  │ B  │ N  │ M  │ ,  │ .  │ /  │   Sft（+Sn）↑ │
     * ├──┬┴─┬──┼───┴───┼─┴──┴──┼────┼───┼───┼────┤
     * │ CL │Win │Alt │    Spc       │     Spc      │Fn(Alt) ｜   ← │  ↓  ｜   →   ｜
     * └──┴──┴──┴───────┴───────┴────┴───┴───┴────┘
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  switch (keycode) {
    ...
    case MY_M:
      {
        bool r = false;
        // 英数モードなら英数のM、NICOLAモードならNICOLAのMを処理する
        uint16_t c = nicola_state() ? NG_M : NG_E_M;
        if (record->event.pressed) {
          // Mキーを押した時の処理
          // CTRLキーが押されているか確認
          uint8_t m = get_mods();
          uint8_t ctrl_mask = MOD_BIT(KC_LCTRL) | MOD_BIT(KC_RCTRL);
          uint8_t cur_ctrl = m & ctrl_mask; // 現在押されているCTRLキー
          if(cur_ctrl && !ctrl_m_pressed) {
            // CTRLキーが押されていたら、CTRLキーを離す→Enterを押す→Ctrlキーを押し直す
            unregister_mods(cur_ctrl);
            register_code(KC_ENT);
            register_mods(cur_ctrl);
            ctrl_m_pressed = true;
        } else {
            // 通常のM押し処理
            r = process_nicola(c, record);
          }
        } else {
          // Mキーを離したときの処理
          if(ctrl_m_pressed) {
            // Mキーを押した時にCtrlが押されていたためEnterを押す処理を行なっていた場合
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        // case KC_HOME:      // Capslockを使いたい場合のために使った覚えがないHOMEキーを採用したがいったん元に戻す
            ...
            return false;
        // 英数モードのとき左親指キー(F14)で、nicola mode オン→右親指に変更。左で変換、右でnicolaオン
        case KC_F14:
            if (record->event.pressed) {
