...
void nicola_m_type(void) {
    ...
    if (n_modifier == 0) {
        switch(nicola_m_key) {
            ...
            //［で、を出す
            case NG_P : send_string("wye"); break; // ゑ
            case NG_LBRC: send_string(","); break; // [  【⌥,】（読点）オプションをつけてみたけど、だめだった。なにも変わらず。以下、・と。も同様
            case NG_RBRC: send_string("]"); break; // ] かっことじ
...
//            case NG_0   : send_string(")"); break;      // ）
            ...
            case NG_SCLN: send_string(";");  break; // 左親指と：で：にしてみた。動くか？？
            ...
            case NG_SLSH: send_string("wyi"); break;     // ゐ
        }
    } else if(nicola_o_key == NG_SHFTR) {
        switch(nicola_m_key) {
